    ...
    with open(vocab_file, 'r') as vocab_f:
      for line in vocab_f:
        ...
        if any(c not in allowed_chars for c in w):
          continue
        # to make sure [] is ok for entity type encoding
        assert '[' not in w
        assert ']' not in w
...
def article2ids(article_words, vocab):
  ...
  Returns:
    ids:
      A list of word ids (integers);
        OOVs are represented by their temporary article OOV number. If the vocabulary size is 50k
        and the article has 3 OOVs, then these temporary OOV numbers will be 50000, 50001, 50002.
    oovs:
...
def show_art_oovs(article, vocab):
  ...
  words = article.split(' ')
  words = [("__%s__" % w) if vocab.word2id(w, None) == unk_token else w for w in words]
  out_str = ' '.join(words)
  return out_str
...
def show_abs_oovs(abstract, vocab, article_oovs):
  ...
  new_words = []
  for w in words:
    if vocab.word2id(w, None) == unk_token: # w is oov
