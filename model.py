...
class SummarizationModel(object):
    ...
    def _reduce_states(self, fw_st, bw_st, scope):
        """
        ...
        with tf.variable_scope(scope):
            ...
            bias_reduce_c = tf.get_variable(
                'bias_reduce_c', [dec_hidden_dim], dtype=tf.float32,
                initializer=self.trunc_norm_init
            )
            bias_reduce_h = tf.get_variable(
                'bias_reduce_h', [dec_hidden_dim], dtype=tf.float32,
                initializer=self.trunc_norm_init
            )
    ...
    def _calc_final_dist(self, vocab_dists, attn_dists):
        ...
        Returns:
            final_dists:
                The final distributions. List length max-dec_steps of (batch_size, extended_vsize)
                arrays.
        """
    ...
    def _add_loss(self, log_dists):
        ...
        Args:
            log_dists: List length max-dec_steps of (batch_size, extended_vsize)
        """
        hps = self._hps
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            # state.
            if hps.two_layer_lstm:
                first_dec_in_state = self._reduce_states(fw_st, bw_st, 'reduce_final_st_1')
                second_dec_in_state = self._reduce_states(fw_st, bw_st, 'reduce_final_st_2')
                # tuple of states, one value per layer
                self._dec_in_state = first_dec_in_state, second_dec_in_state
            else:
                self._dec_in_state = self._reduce_states(fw_st, bw_st, 'reduce_final_st')
...
def _coverage_loss(attn_dists, padding_mask):
    ...
    args:
        attn_dists: the attention distributions for each decoder timestep. a list length
